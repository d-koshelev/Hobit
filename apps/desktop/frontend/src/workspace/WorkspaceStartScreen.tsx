...
export function WorkspaceStartScreen({
  ...
  const layoutClassName = hasRecentWorkspaces
    ? "workspace-start-layout workspace-start-layout-has-recent"
    : "workspace-start-layout workspace-start-layout-empty-recent";
  ...
  const newWorkspacePanel = (
    <form
      aria-labelledby="new-workspace-title"
      className="workspace-start-primary"
      onSubmit={submitWorkspace}
    >
      <div className="workspace-start-copy">
        <h1 className="workspace-start-title" id="new-workspace-title">
          New Workspace
        </h1>
        <p className="workspace-start-text">
          Name a workspace and start with Workspace Agent and Notes. Add Queue,
          Executor, Git, Terminal, or JDBC only when this work needs them.
        </p>
      </div>
      ...
      <div className="workspace-start-actions">
        <Button disabled={isCreatingWorkspace} type="submit" variant="primary">
  ...
  const recentWorkspacesPanel = (
    <section
      aria-labelledby="recent-workspaces-title"
      className="workspace-recent"
    >
      <div className="workspace-section-heading">
        <h2 className="workspace-section-title" id="recent-workspaces-title">
          Recent Workspaces
        </h2>
      </div>
      {isLoadingWorkspaces ? (
        ...
        <div className="workspace-recent-empty">
          ...
          <p className="workspace-recent-empty-text">
            Create a workspace to start building an AI workbench. It will appear
            here the next time you return.
          </p>
        </div>
      )}
    </section>
