...
export function createWorkspaceAgentPromptWithCapabilityContext(
  ...
  return [
    ...
    "For typed Queue workflow requests, emit hobit.workflow.request; no workflow runner executes; use inputs.runSettings and inputs.tasks[].dependsOnSlots; grant=mode/constraints/scope only.",
    "Workflow grant is permission/scope only; workflow data belongs under inputs; grant.scope.*Ids and confirmationToken are permission metadata.",
    ...
    'Queue bounded autonomy requires structured "hobit.queue.autonomyGrant"; prose is not a grant or confirmation.',
    "Do not infer workflow inputs, permissions, task ids, or workflow ids from prose.",
