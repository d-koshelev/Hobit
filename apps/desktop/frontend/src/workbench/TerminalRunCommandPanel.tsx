...
export type TerminalFrameStatusView = {
  label: string;
  variant: "neutral" | "info" | "success" | "warning" | "error";
};

type TerminalRunCommandPanelProps = {
...
export function TerminalRunCommandPanel({
  instance,
  onFrameStatusChange,
  onRunTerminalCommand,
}: TerminalRunCommandPanelProps) {
  ...
  useEffect(() => {
    onFrameStatusChange?.(
      terminalCommandFrameStatusView(runResult, runErrorMessage, isRunning),
    );
  }, [isRunning, onFrameStatusChange, runErrorMessage, runResult]);
  ...
  async function runCommand() {
    ...
    try {
      ...
      setRunErrorMessage(errorToMessage(error, "Unable to run terminal command."));
    } finally {
      setIsRunning(false);
    }
  }
...
export function positiveIntegerInputError(
  value: string,
  label: string,
): string | null {
  const trimmedValue = value.trim();
...
function terminalCommandFrameStatusView(
  ...
  if (errorMessage) {
    return { label: "Request failed", variant: "error" };
  }
