...
- Runtime widget behavior beyond placeholder insertion, the Notes state-save path, Terminal one-shot command runs, Agent Chat proposal-only result persistence, Git manual read-only status refresh, and generic frame/log/layout behavior.
...
- Agent Run runtime, streaming logs, Agent Monitoring read surfaces for stored Agent Chat proposal artifacts, response parsing, response validation, overview summarization, or executor integration beyond the static Agent Monitoring preview.
