...
#[test]
fn executor_path_without_skip_git_repo_check_keeps_exec_args_unchanged() {
    ...
    assert_eq!(
        arg_index(&args, "exec") + 1,
        arg_index(&args, "--output-last-message")
    );
...
"#,
    ...
    assert_eq!(
        ...
        arg_index(&output.command_summary, "--output-last-message")
    );
    assert!(!output
        .command_summary
        .iter()
