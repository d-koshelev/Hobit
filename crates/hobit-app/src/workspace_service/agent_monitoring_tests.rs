use super::*;

use hobit_storage_sqlite::{NewWidgetResult, NewWidgetRun, SqliteStore};

fn initialized_service() -> WorkspaceService {
    let store = SqliteStore::open_in_memory().expect("open in-memory sqlite");
    store.init_schema().expect("initialize schema");
    WorkspaceService::new(store)
}

#[test]
fn agent_monitoring_snapshot_returns_persisted_agent_chat_proposal_result() {
    let service = initialized_service();
    let (workspace_id, workbench_id, widget_id) = add_widget(
        &service,
        AGENT_CHAT_WIDGET_DEFINITION_ID,
        "Agent Chat",
        "core",
    );

    service
        .persist_agent_chat_proposal(agent_chat_proposal_input(
            &workspace_id,
            &workbench_id,
            &widget_id,
        ))
        .expect("persist proposal")
        .expect("proposal run summary");

    let snapshot = service
        .get_agent_monitoring_snapshot(&workspace_id, &workbench_id)
        .expect("read monitoring snapshot")
        .expect("valid workbench snapshot");

    assert_eq!(snapshot.workspace_id, workspace_id);
    assert_eq!(snapshot.workbench_id, workbench_id);
    assert_eq!(snapshot.proposal_results.len(), 1);

    let result = &snapshot.proposal_results[0];
    assert_eq!(result.status, "completed");
    assert_eq!(result.source_widget_id, widget_id);
    assert_eq!(result.source_widget_title, "Agent Chat");
    assert_eq!(result.result_type, AGENT_CHAT_PROPOSAL_RESULT_TYPE);
    assert_eq!(result.runtime_status, AGENT_CHAT_PROPOSAL_RUNTIME_STATUS);
    assert!(result.no_llm_called);
    assert!(result.no_tools_executed);
    assert!(result.no_mutations_performed);
    assert_eq!(result.operator_prompt, "Plan the next Hobit block.");
    assert_eq!(
        result.proposal_summary,
        "Local mock interpreted request: \"Plan the next Hobit block.\""
    );
    assert_eq!(
        result.proposed_plan[0],
        "Review approved workspace metadata."
    );
    assert_eq!(
        result.context_needed[0],
        "No additional context is required for this mock preview."
    );
    assert_eq!(
        result.approved_context_summary,
        "Approved current-view context: Workspace identity."
    );
    assert_eq!(result.approved_context_status, "approved");
    assert_eq!(
        result.approved_context_source_labels,
        vec!["Workspace identity"]
    );
    assert_eq!(result.proposed_actions[0].status, "not_executed");
    assert!(!result.proposed_actions[0].executed);
    assert_eq!(result.safety_notes[0], "Proposal only.");
    assert!(result.raw_payload.contains("\"no_llm_called\":true"));
}

#[test]
fn agent_monitoring_snapshot_for_empty_workbench_returns_empty_results() {
    let service = initialized_service();
    let workspace = service
        .create_empty_workspace("Incident", None)
        .expect("create workspace");
    let workbench_id = workspace.workbench_id.as_deref().expect("workbench id");

    let snapshot = service
        .get_agent_monitoring_snapshot(&workspace.id, workbench_id)
        .expect("read monitoring snapshot")
        .expect("valid workbench snapshot");

    assert!(snapshot.proposal_results.is_empty());
}

#[test]
fn agent_monitoring_snapshot_rejects_cross_workspace_or_workbench_access() {
    let service = initialized_service();
    let first_workspace = service
        .create_empty_workspace("First", None)
        .expect("create first workspace");
    let first_workbench_id = first_workspace
        .workbench_id
        .as_deref()
        .expect("first workbench id");
    let (second_workspace_id, second_workbench_id, second_widget_id) = add_widget(
        &service,
        AGENT_CHAT_WIDGET_DEFINITION_ID,
        "Agent Chat",
        "core",
    );

    service
        .persist_agent_chat_proposal(agent_chat_proposal_input(
            &second_workspace_id,
            &second_workbench_id,
            &second_widget_id,
        ))
        .expect("persist proposal")
        .expect("proposal run summary");

    let empty_first_snapshot = service
        .get_agent_monitoring_snapshot(&first_workspace.id, first_workbench_id)
        .expect("read first snapshot")
        .expect("valid first workbench snapshot");
    let cross_workbench_snapshot = service
        .get_agent_monitoring_snapshot(&first_workspace.id, &second_workbench_id)
        .expect("reject cross-workbench read");

    assert!(empty_first_snapshot.proposal_results.is_empty());
    assert!(cross_workbench_snapshot.is_none());
}

#[test]
fn agent_monitoring_snapshot_does_not_return_terminal_results() {
    let service = initialized_service();
    let (workspace_id, workbench_id, widget_id) =
        add_widget(&service, TERMINAL_WIDGET_DEFINITION_ID, "Terminal", "tool");

    insert_result(
        &service,
        "terminal-run-1",
        "terminal-result-1",
        &widget_id,
        "terminal_command_result",
        r#"{"runtime_status":"terminal_command","stdout":"not monitored here"}"#,
    );

    let snapshot = service
        .get_agent_monitoring_snapshot(&workspace_id, &workbench_id)
        .expect("read monitoring snapshot")
        .expect("valid workbench snapshot");

    assert!(snapshot.proposal_results.is_empty());
}

#[test]
fn agent_monitoring_snapshot_skips_malformed_and_non_matching_payloads() {
    let service = initialized_service();
    let (workspace_id, workbench_id, widget_id) = add_widget(
        &service,
        AGENT_CHAT_WIDGET_DEFINITION_ID,
        "Agent Chat",
        "core",
    );

    insert_result(
        &service,
        "malformed-run",
        "malformed-result",
        &widget_id,
        AGENT_CHAT_PROPOSAL_RESULT_TYPE,
        "{not-json",
    );
    insert_result(
        &service,
        "nonmatching-run",
        "nonmatching-result",
        &widget_id,
        AGENT_CHAT_PROPOSAL_RESULT_TYPE,
        r#"{"runtime_status":"other","no_llm_called":true,"no_tools_executed":true,"no_mutations_performed":true}"#,
    );
    service
        .persist_agent_chat_proposal(agent_chat_proposal_input(
            &workspace_id,
            &workbench_id,
            &widget_id,
        ))
        .expect("persist proposal")
        .expect("proposal run summary");

    let snapshot = service
        .get_agent_monitoring_snapshot(&workspace_id, &workbench_id)
        .expect("read monitoring snapshot")
        .expect("valid workbench snapshot");

    assert_eq!(snapshot.proposal_results.len(), 1);
    assert_eq!(
        snapshot.proposal_results[0].operator_prompt,
        "Plan the next Hobit block."
    );
}

fn add_widget(
    service: &WorkspaceService,
    definition_id: &str,
    title: &str,
    category: &str,
) -> (String, String, String) {
    let workspace = service
        .create_empty_workspace("Incident", None)
        .expect("create workspace");
    let workbench_id = workspace
        .workbench_id
        .as_deref()
        .expect("created workbench id")
        .to_owned();
    let state = service
        .add_widget_instance_to_workbench(
            &workspace.id,
            &workbench_id,
            definition_id,
            title,
            category,
        )
        .expect("add widget")
        .expect("state after add");
    let widget_id = state.widget_instances[0].id.clone();

    (workspace.id, workbench_id, widget_id)
}

fn agent_chat_proposal_input(
    workspace_id: &str,
    workbench_id: &str,
    widget_id: &str,
) -> PersistAgentChatProposalInput {
    PersistAgentChatProposalInput {
        workspace_id: workspace_id.to_owned(),
        workbench_id: workbench_id.to_owned(),
        widget_instance_id: widget_id.to_owned(),
        operator_prompt: "Plan the next Hobit block.".to_owned(),
        approved_context_snapshot_json: r#"{
            "items": [],
            "sourceLabels": ["Workspace identity"],
            "status": "approved",
            "summary": "Approved current-view context: Workspace identity."
        }"#
        .to_owned(),
        proposal: AgentChatProposalInput {
            id: "agent-chat-mock-proposal-1".to_owned(),
            request_summary: "Local mock interpreted request: \"Plan the next Hobit block.\""
                .to_owned(),
            proposed_plan: vec![
                "Review approved workspace metadata.".to_owned(),
                "Prepare an operator-reviewed action preview.".to_owned(),
            ],
            context_needed: vec![
                "No additional context is required for this mock preview.".to_owned()
            ],
            action_proposals: vec![AgentChatProposalActionInput {
                title: "No tool action executed".to_owned(),
                description: "The mock runtime did not call tools.".to_owned(),
            }],
            safety_notes: vec!["Proposal only.".to_owned()],
            runtime_notes: vec![
                "No LLM connected.".to_owned(),
                "No tools executed.".to_owned(),
            ],
        },
    }
}

fn insert_result(
    service: &WorkspaceService,
    run_id: &str,
    result_id: &str,
    widget_id: &str,
    result_type: &str,
    payload: &str,
) {
    service
        .store
        .insert_widget_run(NewWidgetRun {
            id: run_id,
            widget_instance_id: widget_id,
            status: "completed",
            command_kind: Some("test_command"),
            command_payload: None,
            started_at: Some("1.000000000"),
            finished_at: Some("2.000000000"),
            summary: Some("Test run"),
        })
        .expect("insert test run");
    service
        .store
        .insert_widget_result(NewWidgetResult {
            id: result_id,
            run_id,
            status: "completed",
            result_type: Some(result_type),
            summary: Some("Test result"),
            content: Some("Read-only test result"),
            payload: Some(payload),
            created_at: Some("3.000000000"),
        })
        .expect("insert test result");
}
