...
use crate::inputs::NewAgentQueueCompletionDecision;
use crate::mappers::agent_queue_completion_decision_row;
use crate::rows::AgentQueueCompletionDecisionRow;
use crate::time::now_precise_timestamp;

use super::SqliteStore;

impl SqliteStore {
    pub fn insert_agent_queue_completion_decision(
        &self,
        input: NewAgentQueueCompletionDecision<'_>,
        ...
        self.get_agent_queue_completion_decision(
            input.workspace_id,
            input.queue_task_id,
            input.decision_id,
        )?
    ...
    pub fn get_agent_queue_completion_decision(
        &self,
        workspace_id: &str,
        queue_task_id: &str,
        decision_id: &str,
    ) -> Result<Option<AgentQueueCompletionDecisionRow>> {
        self.connection
            .query_row(
                "SELECT
                    decision_id, workspace_id, queue_task_id, run_id, run_link_id,
                    review_message_id, actor_id, decision, reason, metadata_json, created_at
                 FROM agent_queue_completion_decisions
                 WHERE workspace_id = ?1 AND queue_task_id = ?2 AND decision_id = ?3",
                params![workspace_id, queue_task_id, decision_id],
                agent_queue_completion_decision_row,
            )
            .optional()
    }

    pub fn get_agent_queue_completion_decision_by_id(
        &self,
        workspace_id: &str,
        decision_id: &str,
    ) -> Result<Option<AgentQueueCompletionDecisionRow>> {
    ...
    pub fn get_latest_agent_queue_completion_decision(
        &self,
        workspace_id: &str,
        queue_task_id: &str,
    ) -> Result<Option<AgentQueueCompletionDecisionRow>> {
