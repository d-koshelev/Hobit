...
- Queue lifecycle/status vocabulary exists. The visible Sequential Queue
  ...
  Queue now embeds Agent Executor management as the primary Queue execution
  management surface. The embedded section shows local max executors,
  configured workers, spare and working executor slots, worker scopes, dry-run
  capacity recommendations, and safe result/history signals where current
  run-link metadata supports them. The max-executors control is frontend/model
  state in this checkpoint: it bounds explicit Add worker controls, blocks
  lowering below configured workers, and does not start, stop, claim, launch,
  kill, or finalize any runtime work. The standalone Agent Executor widget
  remains present as compatibility/debug/secondary UI and Direct Work runtime
  ownership is unchanged.
  Queue items can now carry a frontend/model structured Worker execution plan
  preview generated by deterministic local heuristics from task prompt/details,
  item type, dependencies, validation hints, likely file/path mentions, and
  assigned/selected worker metadata. The preview shows approximate steps,
  token/time ranges, expected validation commands, likely touched files/areas,
  risk/complexity, stale/ready/split-needed state, and split recommendation.
  It is an estimate, not a guarantee; it is separate metadata rather than
  prompt text, does not duplicate provider/model/thinking/runtime config into
  prompts, and does not start workers, launch Agent Executor/Codex, call
...
- real worker-generated AI planning, worker claiming, or plan-based execution
  gating.
