...
Future agent/task execution observability is defined in `docs/AGENT_RUN_OBSERVABILITY_CONTRACT.md`. The frontend has a static Agent Monitoring placeholder previewing Raw Log, Overview Log, and Result Report sections, and Agent Chat can store proposal-only run/result artifacts, but real run start, Agent Monitoring read surfaces, runtime logs, response parsing, response validation, overview summarization, and executor integration are not implemented yet.
