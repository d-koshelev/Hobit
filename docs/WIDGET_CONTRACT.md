...
Future agent/task execution widgets have an additional observability contract: Raw Log, Overview Log, and Result Report views. See `docs/AGENT_RUN_OBSERVABILITY_CONTRACT.md`. The current Agent Monitoring widget is limited to a read-only viewer for persisted Agent Chat proposal-only artifacts with Overview, Result, and Raw sections; real run observability is not implemented by the current generic Logs panel.
