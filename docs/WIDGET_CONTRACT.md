...
Future agent/task execution widgets have an additional observability contract: Raw Log, Overview Log, and Result Report views. See `docs/AGENT_RUN_OBSERVABILITY_CONTRACT.md`. The current Agent Monitoring widget is only a static placeholder preview of those views; real run observability is not implemented by the current generic Logs panel.
