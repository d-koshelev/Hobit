...
- can start an explicit local desktop Workspace Agent Codex run from the current
  ...
  supported desktop platforms. Browse opens a directory picker only. The
  working directory can still be typed manually when Browse is unavailable or
  canceled. Browse updates only the visible working directory field; it does
  not scan folders, persist the path, or start Codex.
