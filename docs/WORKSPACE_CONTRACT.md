...
- The Agent Chat placeholder has a frontend-local/mock proposal-only prompt, explicit current-session approved context selection, structured preview, and desktop proposal-only widget run/log/result persistence. Selectable context is limited to safe current-view metadata: Workspace/workbench identity, widget inventory metadata, and current global activity status. Agent execution, LLM calls, hidden workspace-context access, persisted approved context models outside the proposal result snapshot, executable action proposals, tool execution, Terminal command execution, Notes body reads, Git status reads, Terminal output reads, widget log reads, filesystem reads, streaming, chat message persistence, Agent Queue item creation by Agent Chat itself, and Workspace content mutation are not implemented. Browser fallback keeps the proposal preview local and reports persistence as unsupported.
- The Agent Monitoring widget is a read-only viewer for persisted Agent Chat proposal-only mock results in the current Workspace Workbench. It keeps the existing `agent-run` definition id for persistence compatibility, shows recent proposal runs plus Overview, Result, and Raw sections for the selected stored artifact, and can explicitly create a review-only Agent Queue item from a valid selected proposal result. Run start, agent execution, terminal execution, streaming, Terminal result monitoring, arbitrary widget result monitoring, response parsing, response validation, overview summarization, Agent Queue execution, and executor integration are not implemented.
- The Agent Queue placeholder has a narrow persisted desktop review inbox for proposal-only Agent Chat mock results. It lists `needs_review` / `pending_review` items scoped to the current Workspace Workbench and shows read-only details; static preview content remains only as empty/demo copy when no persisted items exist. Queue execution, proposal approval/apply behavior, background queue running, automatic launch, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, and executor integration are not implemented.
...
- Agent run observability views defined in `docs/AGENT_RUN_OBSERVABILITY_CONTRACT.md` currently exist only as the Agent Monitoring read-only viewer for Agent Chat proposal-only mock artifacts; real run observability is not implemented.
...
- interactive Terminal, Agent CLI, LLM-backed or executable operational agent chat, or other capability widgets
...
- Agent Queue behavior beyond explicit review-only items created from persisted Agent Chat proposal mock results, queue runner, automatic launch, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, or executor integration
