...
- The Agent Chat placeholder has a frontend-local/mock proposal-only prompt, explicit current-session approved context selection, and structured preview. Selectable context is limited to safe current-view metadata: Workspace/workbench identity, widget inventory metadata, and current global activity status. Agent execution, LLM calls, hidden workspace-context access, persisted approved context snapshots, executable action proposals, tool execution, Terminal command execution, Notes body reads, Git status reads, Terminal output reads, widget log reads, filesystem reads, streaming, chat message persistence, Agent Queue item creation, and Workspace mutation are not implemented.
...
The Coordinator may later propose cross-widget actions, but approved actions must apply through the owning Workspace component and be recorded as auditable Workspace Activity when implemented. Agent Chat / Coordinator does not currently implement approved context access beyond selected current-view metadata, executable action proposals, action execution, Agent Queue item creation, Notebook editing, Git follow-up, or cross-widget mutation.
...
- Workspace-aware Coordinator approved context access beyond selected current-view metadata, persisted approved context snapshots, executable action proposal, preview/approval flow, Agent Queue item creation, Notebook editing, Git follow-up, or cross-widget mutation
