...
- The Widget Catalog can insert the Notes, Terminal, Agent Chat placeholder, Agent Monitoring proposal artifact viewer, Agent Queue placeholder, Git placeholder, and Template Library placeholder as persisted WidgetInstances; other catalog templates remain planned/display-only.
...
- Codex Direct Work has a backend/Tauri one-shot command for explicit Workspace, Workbench, owning widget instance, repository root, operator prompt, sandbox, approval policy, timeout, and output caps. It currently allows the existing Agent Monitoring (`agent-run`) widget definition to own the persisted Direct Work run/log/result artifacts. No Direct Work frontend UI calls this command yet.
- The Workbench top bar includes a compact global activity/idle indicator. Current implementation is frontend/current-session only: it shows no active local runs by default, reflects Terminal one-shot commands started from the current UI session while they are running, and can show a Terminal command attention state after a failed or timed-out current-session request. It does not poll persisted runs, observe background work, monitor external processes, or imply Agent Queue/Agent runtime execution exists.
- The Agent Chat placeholder has a proposal-only prompt, explicit current-session approved context selection, structured preview, a desktop backend AI proposal boundary when an explicit provider is configured, local/mock fallback, and desktop proposal-only widget run/log/result persistence. Selectable context is limited to safe current-view metadata: Workspace/workbench identity, widget inventory metadata, and current global activity status. Agent execution, hidden workspace-context access, persisted approved context models outside the proposal result snapshot, executable action proposals, tool execution, Terminal command execution, Notes body reads, Git status reads, Terminal output reads, widget log reads, filesystem reads, streaming, chat message persistence, Agent Queue item creation by Agent Chat itself, provider settings UI, secrets UI, HTTPS provider adapter, and Workspace content mutation are not implemented. Browser fallback keeps the proposal preview local, does not call providers directly, and reports persistence as unsupported.
- The Agent Monitoring widget is a read-only viewer for persisted Agent Chat proposal-only results in the current Workspace Workbench. It keeps the existing `agent-run` definition id for persistence compatibility, shows recent proposal runs plus Overview, Result, and Raw sections for the selected stored artifact, and can explicitly create a review-only Agent Queue item from a valid selected local mock proposal result. It may own Direct Work run artifacts at the service boundary, but it does not display them yet. Run start UI, terminal execution, streaming, Terminal result monitoring, arbitrary widget result monitoring, response parsing, response validation, overview summarization, Agent Queue execution, and broader executor integration are not implemented.
- The Agent Queue placeholder has a narrow persisted desktop review inbox for Agent Chat local mock proposal results. It lists `needs_review` / `pending_review` items scoped to the current Workspace Workbench and shows read-only details; static preview content remains only as empty/demo copy when no persisted items exist. Queue execution, proposal approval/apply behavior, background queue running, automatic launch, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, and executor integration are not implemented.
...
- real capability widget insertion beyond the Notes, Terminal one-shot command widget, Agent Chat proposal-only mock placeholder, Agent Monitoring proposal artifact viewer, Agent Queue placeholder, Git placeholder, and Template Library placeholder
...
- widget instances
- widget layout
- widget presence zones and future Dock rail placement
- widget states
- widget inputs
...
Future Dock behavior is Workspace-scoped. Dock items are existing WidgetInstances parked in Workspace-local perimeter rails using Indicator view; they are not new templates or duplicate widget instances.

Widgets parked in one Workspace's Dock must not appear in another Workspace. Shared WidgetDefinitions and WidgetTemplates do not imply shared WidgetInstances, Dock placement, widget state, logs, or status. Clicking a Dock item should open Compact view; moving a widget between Canvas, Dock, in-app Float, and future external windows must preserve the same WidgetInstance identity, state, configuration, logs, results, and Workspace ownership.
...
- Queue review, Git review, notes, and Agent Monitoring views must show which Workspace they belong to when opened from cross-Workspace navigation
...
- Agent Queue behavior beyond explicit review-only items created from persisted Agent Chat local mock proposal results, queue runner, automatic launch, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, or executor integration
- real agent run Raw Log, Overview Log, Result Report, log parser, overview summarizer, or response validator beyond the read-only Agent Monitoring proposal artifact viewer
- real capability widget insertion beyond the Notes, Terminal one-shot command widget, Agent Chat proposal-only mock placeholder, Agent Monitoring placeholder, Agent Queue placeholder, Git placeholder, and Template Library placeholder
