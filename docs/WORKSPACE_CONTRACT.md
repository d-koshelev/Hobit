...
- Coordinator Chat is the current user-facing chat/planning surface. It is a local-only placeholder using the existing `interactive-agent` id/component and does not connect to a provider, execute widget tools, persist chat sessions, read hidden context, create Queue tasks, launch Agent Executor, run SQL, mutate Git/files, or run Terminal commands. Older Agent Chat proposal-only AI artifact paths remain compatibility/reference paths, not the current catalog surface.
