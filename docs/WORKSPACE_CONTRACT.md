...
- The Agent Queue placeholder is static and previews future queue/review inbox cards and one representative item detail review surface. Queue item selection, queue storage, real queue item persistence, background queue running, automatic launch, automatic acceptance, response capture/parser/validator, Git association, and executor integration are not implemented.
