...
The current repository contains a root Rust workspace that includes the core crates and the Tauri desktop shell, a Vite/React frontend, a minimal Tauri workspace bridge, and a SQLite workspace persistence foundation. Persisted Notes, a desktop-only one-shot Terminal command widget, Agent Chat local/mock proposal-only placeholder with explicit current-session approved context selection and proposal-only run/result persistence, Agent Monitoring read-only proposal artifact viewer, static Agent Queue placeholder, Git placeholder, and static Template Library placeholder widgets exist as catalog insertion paths. The Git placeholder has a narrow manual desktop-only read-only status refresh path. The Terminal widget has a bounded one-shot command path for persisted Terminal widget instances only, Agent Chat has a frontend-local structured proposal preview that can include selected safe current-view metadata only and store the generated proposal as a structured widget run/log/result artifact in desktop mode, and Agent Monitoring can read only those stored proposal artifacts for the current Workspace Workbench. There is no shell mode, interactive terminal, streaming, PTY, cancellation, command history, agent runtime, Agent CLI runtime, LLM-backed or executable chat runtime, Agent Run runtime, Terminal result monitoring, arbitrary widget result monitoring, Agent Queue runtime/storage, Template Library runtime, Git mutations/diff/log/show, hidden context access, persisted context models outside the proposal result snapshot, or broad tool execution yet.
...
`WORKSPACE_COORDINATOR_AGENT_CONTRACT.md` defines the future Workspace-aware Coordinator Agent model for explicitly approved context reading and previewed cross-widget action proposals. It is a contract only; Agent Chat currently supports only selected current-view metadata snapshots plus proposal-only result persistence and does not read hidden Workspace context, propose executable actions, mutate widgets, create Queue Items, edit Notebook content, or execute actions.
...
`AGENT_RUN_OBSERVABILITY_CONTRACT.md` defines future Raw Log, Overview Log, and Result Report views for agent/task execution. The frontend has an insertable Agent Monitoring widget that displays read-only Overview, Result, and Raw sections for stored Agent Chat proposal-only mock artifacts, and Agent Chat can persist those proposal-only run/result artifacts. Agent Monitoring is not a real Agent Run runtime; agent execution log models, runtime log streaming, Terminal result monitoring, arbitrary widget result monitoring, overview summarizers, response validation, executor integration, and real agent runtime UI are not implemented yet.
...
There is no shell execution, script execution, Script Runner widget beyond planned/display-only catalog metadata, agent runtime, LLM-backed or executable chat runtime, Workspace-aware Coordinator approved context access beyond selected current-view metadata, persisted context models outside Agent Chat proposal result snapshots, executable proposal behavior, Agent Queue behavior beyond the static placeholder preview, Agent Queue storage/runner/real command queue/history/review inbox, real agent run Raw Log/Overview Log/Result Report behavior beyond Agent Monitoring's read-only stored Agent Chat proposal artifact viewer, Terminal result monitoring, arbitrary widget result monitoring, Template Library runtime, template storage/editing/request generation/response validation, Git behavior beyond manual desktop-only read-only status refresh for an explicit transient repository root, real capability widget insertion beyond the Notes, Terminal one-shot command widget, Agent Chat proposal-only mock placeholder, Agent Monitoring proposal artifact viewer, Agent Queue placeholder, Git placeholder, and Template Library placeholder, real Dock behavior, widget Full/Compact/Indicator view mode behavior, persisted presence zones beyond current canvas/floating presentation, preset editor, full drag/drop layout editor, snapping, collision detection, auto-reflow, floating overlay resize, true external Tauri/OS popout window behavior, persisted external popout geometry, always-on-top behavior, or full Notebook/Notes document model yet.
...
The React frontend calls the workspace lifecycle, widget mutation/log read, Git status, Terminal one-shot command, Agent Chat proposal persistence, and Agent Monitoring proposal artifact read paths through the workspace API facade when running inside Tauri. The browser/Vite path uses the same facade with an in-memory implementation; browser fallback throws a visible unsupported state for real Git status reads, Terminal command execution, Agent Chat proposal persistence, and Agent Monitoring persisted artifact reads. The `run_terminal_command` Tauri command is called only from the Terminal widget UI and remains limited to persisted Terminal widget instances. The `persist_agent_chat_proposal` Tauri command is called only from the Agent Chat widget UI and persists proposal-only mock run/log/result artifacts for persisted Agent Chat widget instances. The `get_agent_monitoring_snapshot` Tauri command is read-only, filters to Agent Chat proposal-only mock results in the current Workspace Workbench, and does not expose Terminal results or arbitrary widget results. There is no shell mode, interactive terminal, streaming, PTY, cancellation, command history, agent call, executable chat runtime, Agent Run runtime, Agent Queue runtime/storage, Template Library runtime, Git runtime beyond the narrow read-only status path, workspace restore runtime, log streaming/polling, or settings UI in this milestone.
...
The Agent Chat placeholder accepts an operator prompt, lets the operator explicitly select safe current-session context metadata, and generates a frontend-local/mock structured proposal preview. In the Tauri desktop path, generating a proposal persists a widget run/log/result artifact containing the operator prompt, approved context snapshot used, proposal sections, proposed tool/action items marked not executed, safety notes, and proposal-only safety flags. Selectable context is limited to Workspace/workbench identity, widget inventory metadata, and current global activity status. Browser/Vite fallback reports proposal persistence as unsupported while keeping the local preview visible. Agent Chat does not execute agents, call LLMs, read hidden Workspace context, read Notes body, read Git status, read Terminal output, read widget logs, read files, execute tools, run Terminal commands, create Queue items, stream responses, persist chat messages, persist reusable context snapshots, or write widget state or Workspace content.

The Agent Monitoring widget reads persisted Agent Chat proposal-only mock results for the current Workspace Workbench and renders recent proposal runs plus read-only Overview, Result, and Raw sections for the selected artifact. It keeps the existing `agent-run` definition id for persistence compatibility and does not start runs, stream logs, monitor Terminal results, read arbitrary widget results, parse responses, validate results, summarize runtime events, integrate executor tasks, call agents, execute terminal commands, create Queue items, apply proposals, or write widget state.
...
This storage layer is foundational only. It is wired through `hobit-app` and the Tauri workspace bridge for Workspace lifecycle, Workbench state loading, Notes, Terminal placeholder, Agent Chat placeholder insertion and proposal-only run/result persistence, Agent Monitoring read-only proposal artifact viewing, Agent Queue placeholder, Git placeholder, and Template Library placeholder insertion, Notes placeholder state, persisted widget layout fields, workspace activity events, widget-local logs, Terminal one-shot run/result persistence, and Agent Chat proposal-only run/result persistence. Git status refresh is read-only and does not write repository root/status into storage. The Agent Chat proposal preview and approved context selector remain frontend-local; only the generated proposal result snapshot is stored when the operator generates a proposal in desktop mode. The storage layer is not wired to agent runtime, interactive terminal sessions, LLM-backed or executable chat runtime, persisted context models outside Agent Chat proposal result snapshots, Agent Run runtime, Agent Queue runtime/storage, Template Library runtime, Git runtime beyond the narrow status read path, real capability widgets beyond placeholders, runtime streaming, Terminal result monitoring, or arbitrary widget result monitoring.
...
This application layer is wired to the Tauri workspace bridge. It includes a bounded one-shot Terminal command orchestration path for persisted Terminal widget instances only, creating widget run/log/result records around the shared process adapter. It also includes a proposal-only Agent Chat persistence path for persisted Agent Chat widget instances only, storing the already-generated mock proposal as widget run/log/result records without execution. It does not restore runtime state, execute non-Terminal widgets, run agents, provide shell mode, stream logs, or add executable agent behavior.
...
Full runtime restore is not implemented yet. There is no event replay, widget runtime reconstruction, preset editor, real Dock behavior, widget Full/Compact/Indicator view mode behavior, persisted presence zone model, full drag/drop layout editor, real capability widget insertion beyond the Notes, Terminal one-shot command widget, Agent Chat proposal-only mock placeholder, Agent Monitoring proposal artifact viewer, Agent Queue placeholder, Git placeholder, and Template Library placeholder, shell or interactive terminal execution, LLM-backed or executable chat runtime, persisted approved context models outside Agent Chat proposal result snapshots, Agent Queue execution/storage/real command queue/history/review inbox, Agent Run execution, Template Library execution, Git behavior beyond manual read-only status refresh, or agent runtime behavior.
