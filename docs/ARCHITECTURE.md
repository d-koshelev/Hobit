...
Interactive Agent is a minimal placeholder for manual long-running agent chat/work. It has no Queue integration, no monitoring integration, and no tool execution. Runbook is a minimal placeholder for procedural steps with states such as pending, running, done, failed, skipped, and blocked. It has no step execution, edit mode, builder, Queue integration, or agent-assisted steps.
