...
runtime, Script Runner, JIRA, Confluence, Image Edit, frontend Terminal PTY UI,
Terminal command history, executable chat runtime beyond the manual Direct Work/Queue API paths,
...
runtime/Tauri command foundations now exist for explicit Terminal widget owners
in the desktop shell. Frontend PTY UI, tabs UI, split panes, and
...
The Terminal widget accepts one explicit program, one argument per textarea line, an explicit working directory, timeout, and stdout/stderr caps. In the Tauri desktop path it calls `run_terminal_command`, creates widget run/log/result records through the backend, and renders the final stdout/stderr result. Browser/Vite fallback reports that local process execution is unsupported. This remains the current `Run command` capability until a future PTY/session UI lands. A desktop-only PTY backend/API foundation exists for explicit Terminal widget owners, but no frontend Terminal PTY UI consumes it yet. The current Terminal widget does not implement visible shell mode, interactive stdin, streaming terminal UI, cancellation controls, command history, saved profiles, environment/secrets support, Agent-triggered execution, or Script Runner behavior.
...
Executor, Agent Queue, Coordinator Chat, Database / JDBC, Runbook, Git,
Terminal, and Notes, frontend shell or interactive terminal UI, executable
Coordinator runtime, automatic Agent Queue dispatch or real scheduler behavior
...
form in the current frontend; a backend PTY session foundation exists but has
no visible shell UI yet.
