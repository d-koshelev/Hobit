...
The Agent Chat placeholder accepts an operator prompt, lets the operator explicitly select safe current-session context metadata, and generates a frontend-local/mock structured proposal preview. Selectable context is limited to Workspace/workbench identity, widget inventory metadata, and current global activity status. It does not execute agents, call LLMs, read hidden Workspace context, read Notes body, read Git status, read Terminal output, read widget logs, read files, execute tools, run Terminal commands, create Queue items, stream responses, persist chat messages, persist context snapshots, or write widget/workspace state.
...
This storage layer is foundational only. It is wired through `hobit-app` and the Tauri workspace bridge for Workspace lifecycle, Workbench state loading, Notes, Terminal placeholder, Agent Chat placeholder insertion, Agent Monitoring placeholder, Agent Queue placeholder, Git placeholder, and Template Library placeholder insertion, Notes placeholder state, persisted widget layout fields, workspace activity events, widget-local logs, and Terminal one-shot run/result persistence. Git status refresh is read-only and does not write repository root/status into storage. The Agent Chat proposal preview and approved context selector are frontend-local and do not write storage. The storage layer is not wired to agent runtime, interactive terminal sessions, LLM-backed or executable chat runtime, persisted context snapshots, Agent Run runtime, Agent Queue runtime/storage, Template Library runtime, Git runtime beyond the narrow status read path, real capability widgets beyond placeholders, runtime streaming, or agent run observability storage.
