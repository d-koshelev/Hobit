...
metadata and mock-default read-only query surface backed by workspace-local
JDBC connector metadata storage/API plus widget-owned SQL validation and
bounded mock execution APIs. An opt-in experimental read-only sidecar prototype
exists where current contracts allow it, but there is no credential storage,
production database runtime, production Java sidecar runtime, real `EXPLAIN`,
AI SQL assistance, or Workspace Agent JDBC tool runtime. The future connection
profile boundary is
...
history APIs in a compact read-only history/detail panel. The retained
standalone Git compatibility commit path validates explicit
...
The Agent Queue widget is the Stable v0.1 core dogfooding-loop task organization and execution-follow-up surface for promoted/larger work blocks. Some execution-support behavior remains preview-limited. It is not the default place for every Workspace Agent idea, small task, or quick operator action. Existing proposal-review compatibility paths remain available when review records exist, and the frontend product UI consumes the manual Workspace-scoped task API for create, list, select, edit, status, priority, explicit save, visible Executor assignment, and explicit assigned-task start. Closure outcomes are explicit coordinator/operator decisions: commit created, no-change accepted, follow-up created, or closure blocked / commit required. Report ready is review evidence, not final closure. Automatic dispatch is not implemented. It does not auto-run queue items, approve or apply proposals, launch Terminal, run a background queue, capture responses outside normal Agent Executor artifacts, parse or validate responses, associate Git review, auto-commit, auto-accept, auto-finalize, mutate Notes/Git/files outside the selected Direct Work execution workspace, or write task edits outside explicit task save and assignment actions.
...
manually refreshes a read-only status snapshot through
...
watching, fetch, validation association, checkout/switch branch, revert/reset,
clean, stash, Agent Executor auto-commit, and broader standalone Git
...
mock-default read-only query UI/API without credentials or production database
SQL execution, with an opt-in experimental Java sidecar prototype where
current Preview contracts allow it and backend-only opt-in runtime config
parsing that does not switch the product default away from mock execution,
