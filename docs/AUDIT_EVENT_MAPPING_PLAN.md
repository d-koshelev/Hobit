...
| Agent Queue Sequential Queue Runner | Not audit events yet, except explicit task assignment/start paths it invokes | current-session frontend config and visible operator start exist | no durable runner id, no backend scheduler record, no stable correlation id, no persisted runner lifecycle | runner config may include local paths/prompts; treat as command payload/local path artifact later |
...
- Agent Queue Sequential Queue Runner lifecycle, except the explicit
  assignment/start/completion paths it already invokes.
