...
Current implemented behavior remains the Empty Workbench shell, Workspace lifecycle foundation, Widget Catalog foundation, Notes, a Terminal one-shot local command widget, Direct Work / Codex as a Ready catalog surface backed by the existing `agent-run` widget identity and a backend/Tauri Codex Direct Work one-shot command that persists run/log/result artifacts, Agent Chat proposal-only preview with explicit current-session approved context selection, a first backend AI proposal provider boundary with local/mock fallback, proposal-only run/result persistence, secondary Agent Monitoring read-only proposal artifact details with explicit review-item creation for existing local mock artifacts, Agent Queue proposal-review inbox, Git widget placeholder, and Template Library placeholder insertion paths, persisted widget state/layout foundation, widget-local Logs panel foundation, and workspace activity summaries described in `docs/ARCHITECTURE.md`. The Terminal widget is limited to explicit desktop-only program + argv + working directory runs for persisted Terminal widget instances and does not provide shell mode, interactive terminal behavior, PTY, streaming, cancellation, or command history. Direct Work / Codex accepts an explicit Codex executable in Advanced; when resolving `codex` on Windows the backend also tries `codex.exe`, `codex.cmd`, and `codex.bat` from PATH without invoking a shell. Agent Chat can turn an operator prompt and explicitly selected safe current-view metadata into a structured proposal preview. In the desktop shell, Agent Chat can request a backend AI proposal when `HOBIT_AI_PROVIDER_ENDPOINT` and `HOBIT_AI_PROVIDER_MODEL` configure an explicit `http://` JSON chat-compatible provider endpoint; otherwise it remains fallback-safe. The generated proposal is stored as a structured proposal-only widget run/log/result artifact for the Agent Chat widget. Browser fallback keeps the preview local and does not call providers directly. The secondary Agent Monitoring details can read stored proposal-only Agent Chat artifacts for the current Workspace Workbench and render Overview, Result, and Raw sections with provider status and safety flags; it does not display persisted Direct Work artifacts yet. Agent Queue reads `needs_review` / `pending_review` local mock review items and shows read-only details. Agent Chat, Agent Monitoring, and Agent Queue do not read hidden Workspace context, read Notes body, read Git status, read Terminal output, read widget logs as Coordinator context, read files, execute tools, execute Queue items, persist chat messages, persist reusable approved context snapshots, approve/apply proposals, or mutate Workspace content. The Git widget placeholder supports manual desktop-only read-only status refresh for an explicit transient repository root, and the Template Library placeholder shows static Request, Response, and Coordinator Workflow previews. Provider settings UI, secrets UI, HTTPS provider adapter, Template storage, editing, request generation, response validation, Agent Monitoring Direct Work display, Agent Queue execution/response capture/validation, Git mutations, additional Direct Work UI beyond the current minimal launch surface, and automatic agent execution are not implemented.
