...
- The Agent Chat placeholder has a frontend-local/mock proposal-only prompt and structured preview. It does not call an LLM, execute tools, run Terminal commands, read approved Workspace context, create Agent Queue items, stream responses, persist chat messages, or mutate Workspace, Notes, Git, Queue, widgets, or files.
- Future Workspace-aware Coordinator Agent behavior remains contract-only in `docs/WORKSPACE_COORDINATOR_AGENT_CONTRACT.md`; approved context reading, executable proposal previews, action approval flow, Agent Queue item creation, Notebook editing, Git follow-up, and cross-widget mutation are not implemented.
