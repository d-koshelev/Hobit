...
- Direct Mode is the next contract path for small approved executor work, with Codex CLI as the first planned executor kind and future executor kinds kept agent-agnostic. It is contract-only today: Hobit does not run Codex CLI, start Direct Work runs, execute Queue items, auto-commit, auto-push, mutate Git, add an embedded PTY, or run hidden background agent work.
