...
Near-term agent surface work must follow `docs/AGENT_SURFACE_MODEL.md` and `docs/COORDINATOR_CENTERED_WORKBENCH_CONTRACT.md`. Workspace Agent is the primary operator-facing AI surface; the existing `interactive-agent` id/component is retained as the local Workspace Agent compatibility foundation. Multiple Workspace Agent widgets may exist independently in one Workspace. Workspace Agent plus Agent Queue are the Stable v0.1 core dogfooding loop. Agent Queue organizes promoted async tasks and executor history. Agent Executor uses the existing `agent-run`/Codex Direct Work implementation internally as supporting runtime/detail infrastructure, but it is not a Stable v0.1 product widget. Agent Activity is the current-session readable activity timeline. Runbook remains preview/manual procedural work.
...
- Agent Queue is the Stable v0.1 core dogfooding-loop task organization surface with task create/list/read/update/delete, visible Executor assignment, explicit assigned-task start, safe selected-task Executor run-link history, current-session handoff, final-status auto-refresh, and explicit operator-armed Queue Autorun. Queue Autorun is desktop-local/current-session-only, runs at most one eligible assigned task at a time through supporting Agent Executor runtime while Hobit remains open and the machine remains awake, and has no backend scheduler, durable reconnect/resume, server worker, hidden auto-dispatch, approval/apply behavior, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, or Terminal launch.
