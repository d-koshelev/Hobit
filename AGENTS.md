...
Near-term agent surface work must preserve `docs/AGENT_SURFACE_MODEL.md`. Agent Executor is the current user-facing execution surface and uses the existing `agent-run`/Codex Direct Work implementation internally; Agent Queue is queue/review/history; Interactive Agent and Runbook are separate placeholder surfaces; Coordinator is deferred.
...
- Add Widget opens the Widget Catalog drawer. The current user-facing widget set is Agent Executor, Agent Queue, Interactive Agent, Runbook, Git, Terminal, and Notes. Old Agent Chat, Agent Monitoring, Template Library, Dock, Agent CLI, Script Runner, Database/JDBC, JIRA, Confluence, Image Edit, and Coordinator preview surfaces must not appear in the current catalog or workbench surface.
...
- The Agent Queue placeholder is a review/history foundation only. Queue execution, dispatch, approval/apply behavior, background queue running, automatic launch, automatic acceptance, response capture/parser/validator, Git association, Notes mutation, and executor integration are not implemented.
- The Interactive Agent placeholder is a manual long-running agent chat/work surface direction only. It has no Queue integration, monitoring integration, tool execution, or agent runtime.
...
- Widget frames include a widget-local Logs panel backed by persisted widget logs. Existing widget add/state/layout mutations emit basic logs; Terminal one-shot commands and Codex Direct Work runs emit bounded lifecycle logs. Streaming UI and polling are not implemented.
...
- Do not add widget insertion behavior beyond the current Agent Executor, Agent Queue, Interactive Agent, Runbook, Git, Terminal, and Notes catalog paths unless explicitly requested.
...
- additional widget insertion behavior beyond the current Agent Executor, Agent Queue, Interactive Agent, Runbook, Git, Terminal, and Notes paths
