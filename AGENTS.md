...
- The Agent Chat placeholder has a frontend-local/mock proposal-only prompt, explicit current-session approved context selection, and structured preview. The only approved context sources are safe current-view metadata: Workspace/workbench identity, widget inventory metadata, and current global activity status. It does not call an LLM, execute tools, run Terminal commands, read Notes body, read Git status, read Terminal output, read widget logs, read files, create Agent Queue items, stream responses, persist chat messages, persist context snapshots, or mutate Workspace, Notes, Git, Queue, widgets, or files.
- Future Workspace-aware Coordinator Agent behavior remains contract-only in `docs/WORKSPACE_COORDINATOR_AGENT_CONTRACT.md`; persisted approved context snapshots, executable proposal previews, action approval flow, Agent Queue item creation, Notebook editing, Git follow-up, and cross-widget mutation are not implemented.
